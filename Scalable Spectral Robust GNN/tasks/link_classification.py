...
from tasks.utils import accuracy, link_cls_evaluate, link_cls_train, link_cls_mini_batch_train, link_cls_mini_batch_evaluate
...
class LinkClassification(BaseTask):
    def __init__(self, logger, dataset, model, normalize_times,
        ...
        if self.normalize_times > 1:
            ...
            logger.info("Mean Val ± Std Val: {}±{}, Mean Test ± Std Test: {}±{}".format(round(np.mean(self.normalize_record["val_acc"]), 4), round(np.std(self.normalize_record["val_acc"], ddof=1), 4), round(np.mean(self.normalize_record["test_acc"]), 4), round(np.std(self.normalize_record["test_acc"], ddof=1), 4)))
