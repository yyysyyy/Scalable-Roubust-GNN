...
from datasets.utils import pkl_read_file, download_to, remove_self_loops, coomatrix_to_torch_tensor, edge_homophily, node_homophily, linkx_homophily


class Planetoid(NodeDataset):
    '''
    Dataset description: (Paper with Code): https://paperswithcode.com/datasets
    Directed infomation:    Undirected network from original open source datasets (cora & citeseer & pubmed)
    ...
    split:
        official: 
            ...
            test_idx: 1000
    '''
    def __init__(self, name="cora", root="./datasets/simhomo/", split="official", k=None):
        name = name.lower()
    ...
    def process(self):
        ...
        edge_index = sp.coo_matrix(nx.adjacency_matrix(nx.from_dict_of_lists(graph)))
